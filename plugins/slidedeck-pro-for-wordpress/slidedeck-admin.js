...
var SlideDeckSlides = {
    ...
    addVerticalSlide: function(e){
        e = jQuery(e);
        var max = 10;
        var verticalSlidesContainer = e.parents('li.vertical-editor-area').find('> ul');
        var verticalSlides = verticalSlidesContainer.find('li.vertical-slide');
        var parentSlide = e.parents('.slide');

        // Create array of existing indexes and increment if necessary to prevent ID duplication
        var slideCount = parseInt(verticalSlides.length);
        var existingIndexes = [];
        for(var i=0; i<verticalSlides.length; i++){
            existingIndexes.push(parseInt(verticalSlides.eq(i).find('textarea')[0].id.split('_')[4], 10));
        }
        ...
        if(verticalSlides.length + 1 <= max) {
            jQuery.ajax({
                url: e.attr('href') + "&i=" + slideCount + "&slide_title=" + parentSlide.find('.slide-title').val(),
                success: function(data){
                    // Append the new slide to the list
                    verticalSlidesContainer.append('<li class="vertical-slide"></li>');
                    var verticalSlide = verticalSlidesContainer.find('li.vertical-slide:last')
                        verticalSlide.html(data);
                    
                    var textarea = verticalSlide.find('textarea');
                    var editor_id = textarea.attr('id');
                    var row_id = textarea.parents('.postbox.slide')[0].id;
                    ...
                    for (var i=0; i<textareas.length; i++) {
                        if (!tinyMCE.get(textareas[i].id)) {
                            if(SLIDEDECK_USE_OLD_TINYMCE_EDITOR ===  true){
                                ...
                                QTags._buttonsInit();
                                jQuery('#wp-' + textareas[i].id + '-wrap').removeClass('html-active').addClass('tmce-active');
                            }
                            
                            tinyMCE.init(tinyParams);
                            
                            this.updateUploadInsertLinks(textareas[i]);
                        } else {
                            tinyMCE.get(textareas[i].id).theme.resizeTo(100, 161);
                            // Scale width of editor to 100% since the above resizeTo() does not scale to percentages
                            jQuery("#" + textareas[i].id + "_tbl").width("100%");
                        }
                    }
                }
            ...
            if( confirm("Are you sure you want to remove these vertical slides?\nRemoving the vertical slides will erase this slide's contents.") ){
                ...
                if(!tinyMCE.get(editor.id)){
                    if(SLIDEDECK_USE_OLD_TINYMCE_EDITOR ===  true){
                        ...
                        QTags._buttonsInit();
                        jQuery('#wp-' + editor.id + '-wrap').removeClass('html-active').addClass('tmce-active');
                    }
                    
                    tinyMCE.init(tinyParams);
                    
                    this.updateUploadInsertLinks(editor);
                } else {
                    tinyMCE.get(editor.id).theme.resizeTo(100, 161);
                }
