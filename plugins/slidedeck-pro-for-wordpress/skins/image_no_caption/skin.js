(function($){
	function ImageOnlyNavigation(el){
		var el = $(el);
        ...
        function imgLoaded(el, thisSlide){
            if($.data(el, 'image-no-caption-sized') == true){
                return false;
            }
            ...
            if(pic_real_width > 0 && pic_real_height > 0){
                $.data(el, 'image-no-caption-sized', true);
            }
        }
	...
	$(document).ready(function(){
		for(var i=0, decks=$('.slidedeck_frame.skin-image_no_caption'); i<decks.length; i++){
			var thisDeck = decks[i];
		    
			if(typeof(thisDeck.SlideDeck_skinImageOnlyNavigation) == 'undefined'){
				thisDeck.SlideDeck_skinImageOnlyNavigation = ImageOnlyNavigation(thisDeck);
			}
		}
	});
})(jQuery);