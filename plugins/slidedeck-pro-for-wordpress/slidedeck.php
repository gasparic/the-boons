...
define( 'SLIDEDECK_VERTICAL_TITLE_LENGTH',          20 ); // characters
...
function slidedeck_install() {
    ...
    wp_remote_fopen( "http://trk.kissmetrics.com/e?_k=" . KMAPI_KEY . "&_n=SlideDeck+Activated&_p=" . SLIDEDECK_USER_HASH . '&version=Pro' );
}
function slidedeck_deactivate() {
    wp_remote_fopen( "http://trk.kissmetrics.com/e?_k=" . KMAPI_KEY . "&_n=SlideDeck+Deactivated&_p=" . SLIDEDECK_USER_HASH . '&version=Pro' );
}
...
function slidedeck_menu() {
    ...
    $show_menu = true;
    if( $slidedeck_global_options['disable_edit_create'] == true ) {
...
/**
 * Get SlideDeck Skin Transitions
 * 
 * Returns an array of the available slide transitions for the user's SlideDecks
 * 
 * @return array
 */
function slidedeck_slide_transitions() {
...
function slidedeck_get_dynamic_option( $slidedeck, $option ) {
    ...
    $defaults = array( 
        ...
        'title_length_vertical' => SLIDEDECK_VERTICAL_TITLE_LENGTH,
        'feed_url' => "",
        'validate_images' => 0
    );
...
function slidedeck_add_new() {
    ...
    $skins = slidedeck_get_skins();
    $slide_transitions = slidedeck_slide_transitions();
...
function slidedeck_dynamic() {
    ...
    $post_types = array();    
    //pulling post types to narrow down which ones to show in Type of Content
    if ( function_exists('get_post_types') ){
        
        //getting all post types to compare between publicly_queryable and public
        $all_post_types = get_post_types( array(), 'objects');        
        $publics_defaults_not_set = array();
        
        foreach( $all_post_types as $all_post_type ){
            $is_publicly_queryable = $all_post_type->publicly_queryable;
            $is_public = $all_post_type->public;
            
            if( !isset($all_post_type->publicly_queryable) && !isset($all_post_type->public) ){
                $publics_defaults_not_set[] = true;
            }
            
            //if post type is publicly queryable OR post type is not publicly query AND is public - WP 3.2 post type of pages publicly queryable is not set, but is set to public
            if( $is_publicly_queryable || ( !$is_publicly_queryable && $is_public) ){
                $post_type_name = $all_post_type->name; 
                
                //verifying that attachment, media, revision, and nav_menu_item never appears
                if( !in_array($post_type_name, array("attachment","media","revision","nav_menu_item") ) ) {
                     $label_name = $all_post_type->labels->name;
                     if( !isset( $all_post_type->labels->name ) || empty( $all_post_type->labels->name ) ){
                         $label_name = $all_post_type->name;
                     }                     
                     $post_types[] = array(
                        'name' => $all_post_type->name,
                        'label' => $label_name
                    );
                }
            }
        }
        if( in_array( true, $publics_defaults_not_set ) ){
            //left in for catchall for 2.9 posts and pages since 'publicly_queryable' is not set by default
            $valid_post_types = array( 'post', 'page' );
            
            foreach( (array) $valid_post_types as $valid_post_type ) {
                $post_types[] = array(
                    'name' => $valid_post_type,
                    'label' => ucwords( $valid_post_type ) . "s"
                );
            }
        }        
    }else{
        //left in for catchall 2.8
        $valid_post_types = array( 'post', 'page' );
    ...
    switch ( $action ) {
        case "edit":
            ...
            for ( $i=0; $i<count( $slides ); $i++ ) {
                ...
                $slides[$i]['background'] = isset( $slide_backgrounds[$slides[$i]['id']] ) ? $slide_backgrounds[$slides[$i]['id']] : "";
            }
        ...
        case "delete":
            if( check_admin_referer( 'slidedeck-delete' ) ) {
                ...
                wp_remote_fopen( "http://trk.kissmetrics.com/e?_k=" . KMAPI_KEY . "&_n=SlideDeck+Deleted&_p=" . SLIDEDECK_USER_HASH . '&variation=' . ( $params['dynamic'] == '1' ? 'dynamic' : 'manual' ) . '&version=Pro' );
            } else {
                return false;
            }
        ...
        default:
            ...
            if( isset( $_POST ) && !empty( $_POST ) ) {
                ...
                $options = array( 
                    ...
                    'disable_edit_create' => isset( $_POST['disable_edit_create'] ) ? true : false
                );
...
function slidedeck_save( $post_params = null, $post_status = 'publish' ) {
                    ...
                    foreach ( (array) $val1 as $key2 => $val2 ) {
                        ...
                        switch ( $key2 ) {
                            ...
                            case "vertical_content":
                                $sanitized = array();
                                foreach( $val2 as $v_slide ) {
                                    $sanitized[] = html_entity_decode( slidedeck_sanitize( $v_slide ), ENT_QUOTES, 'UTF-8' );
                                }
                                $sanitized = $sanitized;
                            break;
                        }
    ...
    if ( !isset( $params['slidedeck_options']['useNewVertical'] ) ) {
        $params['slidedeck_options']['useNewVertical'] = 'false';
    }
    if ( !isset( $params['slidedeck_options']['slideTransition'] ) ) {
        $params['slidedeck_options']['slideTransition'] = 'slide';
    }
    if ( !isset( $params['slidedeck_options']['touch'] ) ) {
        $params['slidedeck_options']['touch'] = 'false';
    }
    ...
    if( $params['dynamic'] != '1' ) {
        // If the slide is a vertical slide, serialize it and replace "content"
        $vertical_serialize_counter = 1;
        foreach( (array) $params['slide'] as $slide ) {
            if( $slide['vertical'] == 1 ) {
                $params['slide'][$vertical_serialize_counter]['content'] = serialize( array( 'contents' => $slide['vertical_content'], 'titles' => $slide['vertical_title'] ) );
            }
            $vertical_serialize_counter ++;
        }
    }
    
    // Array to be populated with row IDs for slides that contain vertical slide content
    $vertical_slides = array();
    
    // Array to be populated with row IDs for slides that have a background image defined
    $slide_backgrounds = array();
    ...
    switch ( $action ) {
        case "create":
            ...
            if( $post_status == "publish" ) {
                wp_remote_fopen( "http://trk.kissmetrics.com/e?_k=" . KMAPI_KEY . "&_n=SlideDeck+Created&_p=" . SLIDEDECK_USER_HASH . '&variation=' . ( $params['dynamic'] == '1' ? 'dynamic' : 'manual' ) . '&version=Pro' );
            }
            ...
            if ( $params['dynamic'] != '1' ) {
                ...
                foreach ( (array) $params['slide'] as $slide ) {
                    ...
                    // Add the vertical slide id to the array
                    if( $slide['vertical'] == 1 ) {
                         $vertical_slides[] = $slide_id;
                    }
                    
                    // Add the slide id to the array if a background is defined
                    $slide_backgrounds[$slide_id] = $params['slidedeck_options']['slide_backgrounds'][$count];
                    $count++;
                }
            }
        break;
    ...
    // Unserialize
    $slidedeck_params['slidedeck_options'] = unserialize( $slidedeck_params['slidedeck_options'] );
    
    // Only add this option to the options array if there are vertical slides.
    if( isset( $vertical_slides ) && !empty( $vertical_slides ) ) {
        $slidedeck_params['slidedeck_options']['vertical_slides'] = implode( ',', $vertical_slides );
    }
    
    $slidedeck_params['slidedeck_options']['slide_backgrounds'] = $slide_backgrounds;
    
    // Serialize
    $slidedeck_params['slidedeck_options'] = serialize( $slidedeck_params['slidedeck_options'] );
...
function slidedeck_print_scripts() {
    ...
    wp_register_script( 'slidedeck-library-js', slidedeck_url( '/lib/slidedeck.jquery.js' ), array( 'jquery' ), SLIDEDECK_VERSION );
...
function slidedeck_process_template( $slidedeck_id, $styles = array( 'width' => '100%', 'height' => '300px' ), $include_skin_files = true ) {
    ...
    if ( isset( $slidedeck ) && !empty( $slidedeck ) ) {
        ...
        if ( $is_dynamic === true ) {
            $args = array(
                'post_type' => slidedeck_get_dynamic_option($slidedeck, 'post_type'),
                'post_status' => 'publish',
                        ...
                        switch( $feed_format ) {
                            case 'atom':
                                $slide_nodes = array(
                                    ...
                                    'timesince' => "Posted " . human_time_diff( strtotime( $item->published ) ) . " ago",
                            ...
                            case 'rss':
                                ...
                                $slide_nodes = array(
                                    ...
                                    'timesince' => "Posted " . human_time_diff( strtotime( $date ) ) . " ago",
                                    'permalink' => $item->link
                                );
        ...
        foreach ( (array) $slidedeck['slidedeck_options'] as $key => $val ) {
            ...
            elseif ( is_array( $val ) ) {
                ...
                foreach( $val as $val_key => $val_val ) {
                    $slidedeck_options_json.= $val_sep . $val_key . ": " . $val_val;
                    $val_sep = ", ";
                }
                $slidedeck_options_json.= '}';
            }
    ...
    if( !$vertical_smart_skin ){
        foreach ( (array) $slides as $slide ) {
            ...
            if( $is_vertical ) {
                ...
                $use_new_dl_structure = (boolean) ( slidedeck_get_option( $slidedeck, 'useNewVertical' ) == 'true' );
                
                $vertical_html_tags = array(
                    'container' => 'ul',
                    'item' => 'li'
                );
                ...
                for( $vc = 0; $vc < count( $vertical_content ); $vc++ ) {
                    ...
                    $template_str.= "<{$vertical_html_tags['item']}>" . slidedeck_process_slide_content( $vertical_slide, false, $slidedeck['new_format'] ) . "</{$vertical_html_tags['item']}>";
                }
                $template_str.= "</{$vertical_html_tags['container']}>";
            } else {
                $template_str.= slidedeck_process_slide_content( $slide['content'], false, $slidedeck['new_format'] );        // Slide Content
            }
...
function slidedeck_add_custom_box() {
    if ( function_exists( 'add_meta_box' ) ) {
        ...
        if ( function_exists('get_post_types') ){        
            ...
            foreach( $all_post_types as $all_post_type ){
                ...
                if( $is_publicly_queryable || ( !$is_publicly_queryable && $is_public) ){
                    ...
                    //verifying that attachment, media, revision, and nav_menu_item never appears
                    if( !in_array($post_type_name, array("attachment","media","revision","nav_menu_item") ) ) {
                         $valid_post_types[] = $post_type_name;
                    }
                }
            }
            if( in_array( true, $publics_defaults_not_set ) ){
        ...
        foreach( $valid_post_types as $valid_post_type ){
            add_meta_box( 'slidedeck-sidebar', 'SlideDeck', 'slidedeck_cusom_box', $valid_post_type, 'side', 'high' );
        }
    }
}
...
/**
 * AJAX function for adding a new vertical slide
 * 
 * @param int $_GET['count'] The count of the parent horizontal slide
 * @param int $_GET['i'] The count of the current displayed vertical slides
 * 
 * @return the formatted slide editing area as templated in views/_edit-vertical-slide.php
 */
function slidedeck_add_vertical_slide() {
    global $post_ID;
    ...
    $slide = array(
        'title' => ( !empty( $_GET['slide_title'] ) ? $_GET['slide_title'] : "Slide " . $count ),
        'slide_order' => $count,
        'gallery_id' => $_GET['gallery_id']
    );
    ...
    include( slidedeck_dir( '/views/_edit-vertical-slide.php' ) );
    exit;
}
...
if ( function_exists( 'add_action' ) ) {
    ...
    add_action( 'wp_ajax_slidedeck_add_vertical_slide', 'slidedeck_add_vertical_slide' );
